...
# Distribute 10 workspaces across active monitors so each screen owns its own.
active_mons=()
case "$n" in
  0) active_mons=("eDP-1") ;;
  1) active_mons=("eDP-1" "${externals[0]}") ;;
  *) active_mons=("${ordered[@]}") ;;
esac
...
for idx in "${!active_mons[@]}"; do
  mon="${active_mons[$idx]}"
  count=$per
  [[ $idx -lt $rem ]] && count=$((count + 1))
  first=1
  for ((i = 0; i < count; i++)); do
    if [[ $first -eq 1 ]]; then
      ws_batch+=("keyword workspace $ws,monitor:$mon,default:true,persistent:true")
      first=0
    else
      ws_batch+=("keyword workspace $ws,monitor:$mon,persistent:true")
    fi
    # Move the workspace if it already exists elsewhere.
    hyprctl dispatch moveworkspacetomonitor "$ws $mon" >/dev/null 2>&1 || true
    ws=$((ws + 1))
  done
done

if [[ ${#ws_batch[@]} -gt 0 ]]; then
  joined_ws="$(printf ' ; %s' "${ws_batch[@]}")"
  hyprctl --batch "${joined_ws# ; }" >/dev/null
fi
