...
launch_all() {
  ...
  # NOTE: ~/.config/gtk-3.0/gtk.css paints `window` #000 at USER priority
  # (800), above eww's own CSS (600); eww surfaces stay transparent only
  # via the `window.background:not(.csd)` exception in that file.
...
# The render check can pass while the socket FILE is dead: an eww CLI
...
try:
    ...
    sys.exit(1)
EOF
  done < <(ss -xlp 2>/dev/null | grep -F '("eww"' | grep -oE '/[^ ]+/eww-server_[^ ]+' | sort -u)
  [ "$ok" = 1 ]
}
...
verify_socket || { launch_all; verify_socket || true; }
