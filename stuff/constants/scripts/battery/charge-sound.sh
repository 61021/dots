...
# Event names or absolute paths; kw-sound resolves the theme (Ocean > freedesktop)
# and honors the global "kw-sound off" switch.
SOUND_PLUG="${SOUND_PLUG:-power-plug}"
SOUND_UNPLUG="${SOUND_UNPLUG:-power-unplug}"

play() { ~/.local/bin/kw-sound "$1" >/dev/null 2>&1 & }
